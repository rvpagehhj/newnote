...
public class NoteServiceImpl implements NoteService {
    ...
    public ApiResponse<List<NoteRankListItem>> submitNoteRank() {
        String cacheKey = "note:rank:list";

        // 1. 先查 Redis 缓存
        List<NoteRankListItem> cachedList = (List<NoteRankListItem>) redisService.get(cacheKey);
        if (cachedList != null && !cachedList.isEmpty()) {
            return ApiResponseUtil.success("获取笔记排行榜成功", cachedList);
        }

        // 2. 缓存未命中，查数据库
        List<NoteRankListItem> rankList = noteMapper.submitNoteRank();

        // 3. 写入 Redis 缓存（设置过期时间：5分钟）
        redisService.setWithExpiry(cacheKey, rankList, 300);

        return ApiResponseUtil.success("获取笔记排行榜成功", rankList);
    }
    ...
    public ApiResponse<List<NoteHeatMapItem>> submitNoteHeatMap() {
        Long userId = requestScopeData.getUserId();
        String cacheKey = "note:heatmap:user:" + userId;

        // 1. 先查 Redis 缓存
        List<NoteHeatMapItem> cachedData = (List<NoteHeatMapItem>) redisService.get(cacheKey);
        if (cachedData != null && !cachedData.isEmpty()) {
            return ApiResponseUtil.success("获取笔记热力图成功", cachedData);
        }

        // 2. 缓存未命中，查数据库
        List<NoteHeatMapItem> heatMap = noteMapper.submitNoteHeatMap(userId);

        // 3. 写入 Redis 缓存（设置过期时间：1分钟）
        redisService.setWithExpiry(cacheKey, heatMap, 60);

        return ApiResponseUtil.success("获取笔记热力图成功", heatMap);
    }
